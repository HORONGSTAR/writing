...
import { Container, Box, Tab, List, ListItem, Typography, Divider } from '@mui/material'
   ...
   return (
      <Box sx={{ width: '100%', typography: 'body1' }}>
         <TabContext value={value}>
            ...
            <TabPanel value="1">{user?.Posts.length > 0 ? <PostItem posts={user?.Posts} /> : <NoticeBox>등록된 글이 없습니다.</NoticeBox>}</TabPanel>
            <TabPanel value="2">
               {user?.Posts.length > 0 ? <PostItem posts={user?.BookmarkPost} /> : <NoticeBox>등록된 글이 없습니다.</NoticeBox>}
            </TabPanel>
            <TabPanel value="3">
